...
for i in range(0, 10):
    ...
    with open(f'En1_select_random_{i}/diversity.csv') as csv_file:
...
for i in range(0, 19):
    ...
    y_err = np.sqrt(sum([data_std['run0'][i]**2, data_std['run1'][i]**2, data_std['run2'][i]**2, data_std['run3'][i]**2, \
        data_std['run4'][i]**2, data_std['run5'][i]**2, data_std['run6'][i]**2, data_std['run7'][i]**2, data_std['run8'][i]**2, data_std['run9'][i]**2 ]))
